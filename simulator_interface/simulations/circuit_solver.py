...
def simulation_iterations(sim_id, synthesized_ckt_comps):
    ...
    # Generating the plot key file for external plotting
    plotkey_file_path = os.path.join(os.sep, \
                                sim_para_model.sim_working_directory, \
                                "plotkey.txt")

    plotkey_file_object = open(plotkey_file_path, "w")

    plotkey_file_object.write("t = 1")
    plotkey_file_object.write("\n")
    plt_key_count = 2
    for c1 in range(len(meter_list)):
        plotkey_file_object.write("%s = %d" %(component_objects[meter_list[c1]].type+'_'+component_objects[meter_list[c1]].tag, plt_key_count),)
        plotkey_file_object.write("\n")
        plt_key_count += 1
    ...
    if plotted_variable_list:
        for c1 in plotted_variable_list:
            if control_file_variablestorage[c1][1].lower()=="yes" or \
                    control_file_variablestorage[c1][1].lower()=="y":
                plotkey_file_object.write("%s = %d" %(c1, plt_key_count),)
                plotkey_file_object.write("\n")
                plt_key_count += 1

    plotkey_file_object.close()
