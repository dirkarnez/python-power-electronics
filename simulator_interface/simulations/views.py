...
class ExportPlots(RedirectView, SimulationData):
    """
    This class exports the plots defined into a Matplotlib file that
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        plot_list, plotlines_list = self.get_plot_data()
        all_plots = []
        for plot_list_item in plot_list:
            plot_item_obj = {
                'title': plot_list_item.plot_title
            }
            plot_item_obj['waveforms'] = plot_list_item.circuitwaveforms_set.all()
            plot_item_obj['plots'] = []
            for plot_waveform_item in plot_list_item.circuitwaveforms_set.all():
                waveforms_in_plot = plot_waveform_item.waveform.all()
                if len(waveforms_in_plot) > 0:
                    waveforms_in_plot = waveforms_in_plot[0]
                if waveforms_in_plot:
                    plot_item_obj['plots'].append(waveforms_in_plot)
            all_plots.append(plot_item_obj)
        ...
        if all_plots and len(all_plots) > 0:
            ckt_param_path = os.path.join(os.sep, \
                                ...
                                'auto_plot.py')
            try:
                plot_file_obj = open(ckt_param_path, 'w')
            except:
                error_message = 'Cannot create plot file. Make sure file with that name is not open already.'
                print(error_message)
            else:
                error_message = None

                plot_file_obj.write('import numpy as np')
                plot_file_obj.write('\n')
                plot_file_obj.write('import matplotlib')
                plot_file_obj.write('\n')
                plot_file_obj.write('import matplotlib.pyplot as plt')
                plot_file_obj.write('\n')
                plot_file_obj.write('import pandas as pd')
                plot_file_obj.write('\n\n')
                plot_file_obj.write('df = pd.read_csv("' + self.sim_para_model.sim_output_file +'", delim_whitespace=True)')
                plot_file_obj.write('\n')
                plot_file_obj.write('df.columns = list(range(len(df.columns)))')
                plot_file_obj.write('\n\n')
                plot_file_obj.write('font = {"family" : "normal",')
                plot_file_obj.write('\n')
                plot_file_obj.write('"weight" : "normal",')
                plot_file_obj.write('\n')
                plot_file_obj.write('"size"   : 12}')
                plot_file_obj.write('\n')
                plot_file_obj.write('matplotlib.rc("font", **font)')
                plot_file_obj.write('\n\n')
                
                for plot_item_index, plot_item in enumerate(all_plots):
                    plot_file_obj.write('plt.figure()')
                    plot_file_obj.write('\n')
                    plot_file_obj.write('plt.title("' + plot_item['title'] + '")')
                    plot_file_obj.write('\n')
                    plot_file_obj.write('plt.xlabel("Time")')
                    plot_file_obj.write('\n')
                    for count1 in range(len(plot_item['plots'])):
                        plot_file_obj.write('plt.plot(np.array(df[0])[:, None], np.array(df[' + str(plot_item['plots'][count1].line_pos) + '])[:, None], label = "' + plot_item['waveforms'][count1].waveform_legend + '")')
                        plot_file_obj.write('\n')
                    plot_file_obj.write('plt.legend()')
                    plot_file_obj.write('\n')
                    plot_file_obj.write('plt.savefig("fig' + str(plot_item_index + 1) + '.eps", format="eps")')
                    plot_file_obj.write('\n\n')

                plot_file_obj.write('plt.show()')
                plot_file_obj.write('\n')
                plot_file_obj.close()
